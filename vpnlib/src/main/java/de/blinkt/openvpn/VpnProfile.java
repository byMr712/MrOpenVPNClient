...
public class VpnProfile implements Serializable, Cloneable {
    ...
    public String getConfigFile(Context context, boolean configForOvpn3) {
        ...
        if (!configForOvpn3) {
            ...
            if (mUseLegacyProvider || usesLegacyCipher())
    ...
    public boolean usesLegacyCipher() {
        String legacyCiphers = "";
        if (!TextUtils.isEmpty(mCipher))
            legacyCiphers = mCipher;
        if (!TextUtils.isEmpty(mDataCiphers))
            legacyCiphers = legacyCiphers + ":" + mDataCiphers;
        legacyCiphers = legacyCiphers.toUpperCase(Locale.ROOT);
        return legacyCiphers.contains("BF-CBC")
                || legacyCiphers.contains("DES-CBC")
                || legacyCiphers.contains("DES-EDE")
                || legacyCiphers.contains("RC2-")
                || legacyCiphers.contains("RC4")
                || legacyCiphers.contains("SEED-")
                || legacyCiphers.contains("IDEA-");
    }
